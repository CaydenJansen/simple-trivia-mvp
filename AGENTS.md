...
- Live bonuses use a staged flow within their parent question: main answer first, a visible bonus-coming cue, then the host-controlled bonus appears alongside the still-editable main answer, followed by one close action that locks both stages. Store bonus submissions and grading separately from ordinary submissions, then reveal and score both together exactly once.
