...
drop function if exists public.save_my_question_with_metadata(uuid, jsonb, uuid, uuid[], uuid[]);

create function public.save_my_question_with_metadata(
  p_question_id uuid,
  p_question jsonb,
...
begin
  ...
  if p_question_id is null then
    insert into public.source_questions (
      ...
      'user', auth.uid(), p_question->>'question_type', btrim(p_question->>'prompt'),
...
revoke all on function public.save_my_question_with_metadata(uuid, jsonb, uuid, uuid[], uuid[], jsonb) from public;
grant execute on function public.save_my_question_with_metadata(uuid, jsonb, uuid, uuid[], uuid[], jsonb) to authenticated;

comment on function public.save_my_question_with_metadata(uuid, jsonb, uuid, uuid[], uuid[], jsonb) is
  'Atomically saves one owned reusable question, its optional one-to-one bonus, and controlled metadata while maintaining legacy runtime projections.';

comment on column public.source_question_bonuses.image_url is
  'Compatibility URL projection for optional bonus media until all authoring flows use media_assets.';
