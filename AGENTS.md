...
- My Quizzes folders are owner-scoped, sync across devices, and organise quiz definitions without affecting live games or shared copies. Desktop cards can be dragged between folders; touch and keyboard users retain an explicit Move to folder control. Deleting a folder moves its quizzes to Unfiled rather than deleting them.
- A host may create one permanent venue QR code from Account Settings. Its stable public link resolves only to that host's newest lobby or live game; when none exists, players see a neutral waiting screen that keeps checking. Never expose the host identity or an ended/cancelled game through this link.
- A host may save a quiz as a reusable show template. Creating a new show from that template preserves rounds, item order, content screens, and games while replacing every ordinary trivia question with a different active verified Question Library item. Question slots accept any supported mechanic by default and may be restricted by the host. Fail clearly rather than retaining an old question when the library cannot fill the structure.
...
- Template question placeholders use the purple Question Library treatment and default to **Any question type**. Hosts may optionally restrict an individual slot to a supported question mechanic. The template editor mirrors the regular Quiz Builder's round cards and drag-to-reorder interaction rather than introducing a separate editor language.
- Template use also replaces every backup tiebreaker, in-show tiebreaker, and Tiebreaker-style Question game with a different active verified Tiebreaker Library item. Preserve the slot's mode, show position, and reward settings while replacing its prompt, answer, unit, notes, and source reference.
- When a template has recognised topic-based round titles, using it asks whether to keep those round topics or randomly assign different viable topics. In either mode, fill question slots with fresh unused Question Library questions matching the resulting topic and preserve all non-question structure.
...
- Backup tiebreakers, in-show tiebreakers, and Tiebreaker-style Question games expose their library replacement and **Try another** actions on the right side of their overview cards, matching ordinary question-card controls.
- Template tiebreaker slots are purple **Tiebreaker Library** placeholders in both previews and the template editor. In-show and prize-bearing Tiebreaker-style slots receive fresh random Tiebreaker Library content when used, and the template editor offers the same default in-show **Add Tiebreaker** action as Quiz Builder.
- Quiz Builder supports inserting questions, content screens, and show games between existing round items. Items can be dragged within a round or into another round while preserving one collision-free global item order. Dragging near the viewport edge must auto-scroll so long rounds remain reorderable.
- Quiz Builder lets a host retheme a generated round from its round header. Retheming replaces only that round's ordinary Question Library questions with fresh unused library questions matching the chosen topic and existing question mechanics; preserve games, content screens, tiebreakers, item order, and points.
- Round insertion controls use persisted item positions, which may be global across the quiz. Footer actions insert after the round's greatest item position, and between-item actions insert immediately after the selected item; never derive either position from the round-local array index or item count.
...
- Auto-Build may optionally add exactly one active random-chance show game at the end of each round. It may choose Spin the Wheel, Heads or Tails, or Dodge the Rock; it must never auto-author an Audience Question. Beat the Bomb, Big Balloon, and Steal the Treasure are archived: preserve their implementation and existing-show compatibility, but never offer them for new authoring, templates, tie resolution, or automatic generation. All generated games share the host's selected points or custom-prize reward, and each game adds three minutes to the estimated runtime. Remember the complete Generate-menu setup on that browser, including whether games are enabled and their shared reward.
...
- Auto-Build vibe filters include **Uber Dweeb** for geography, history, science/chemistry and traditional hard-trivia subjects, and **Pop Culture Girly** for film, television, music, celebrities and pop culture. These are hard eligibility filters, like the existing vibe choices, and never restrict prepared tiebreakers.
...
- Auto-Run pauses after an in-show tiebreaker resolves so the host can review every submitted answer and closest-distance result before explicitly continuing; it must never skip directly from the tiebreaker to the end of the show.
...
- Beat the Bomb is a random-chance interlude: the bomb visibly arms for 20 seconds, then may explode at any time in the next 60 seconds. It cannot explode before at least one team cuts; if nobody cuts by the end of the window, overtime waits for the first cut so the game always has a winner. Each team cuts once, players cannot see other teams' cuts during play, and the latest server-timestamped safe cut wins. Result screens compare each team's cut with the authoritative explosion time and show the winner's timing to losing teams. Spin the Wheel freezes all joined teams when it starts, shows the team currently under the pointer, decelerates to exactly the server-selected team, and withholds every winner/result message until that landing animation finishes. Both use the same frozen reward model: either bonus points applied exactly once to the winner's score or a custom prize that never alters score; show the configured reward before play and custom winner copy only to the winning team.
- Deal or No Deal assigns hidden bank cases from $1–$93. Each bank swap must move the case value by at least five, avoid a duplicate live case whenever capacity permits, and remain private until the final reveal.
- Shared Cursor is server-authoritative and every player and host must see its position update live without needing to tap. Use a recognizable mouse-pointer graphic. Each team has five tap-stamina units with passive recovery; exhausting all five triggers a server-enforced three-second cooldown shown with a visible stamina bar.
...
- Player answer-entry screens place their submit control immediately after the answer controls so it appears as high as the content allows; do not pin it to the viewport bottom or place it after nonessential whitespace.
- Realtime updates from other teams must never overwrite or clear an in-progress local answer draft. When a live show-game record is still hydrating after refresh, render a neutral loading state rather than any specific game's fallback UI.
- Bind every core and bonus submission atomically to the question key shown when the player submitted. A late request after host progression must be rejected rather than recorded against the next question. Rapid repeated taps or Return presses must result in one in-flight request, and locked Audience Question responses must be idempotent.
- Guard overlapping realtime hydrations so a slower stale response cannot overwrite newer question, submission, or show-game state.
...
- Steal the Treasure is server-authoritative and lasts about 40 seconds. Players must always see a prominent live countdown, the current highest banked score, and the steal button without scrolling when play opens on a normal mobile viewport. A hold has a 300ms warm-up before any treasure accrues; holding after that while the guard sleeps accumulates an unbanked haul, and releasing banks it. Guard sleep and wake windows vary substantially between transitions. A player still holding when the guard wakes loses only the unbanked haul. The highest banked total wins, and fast mobile press/release sequences must preserve the player's final intent.
...
- Randomize each unanswered ranking question before presenting it, and never initialize it in the authored correct order.
