...
begin
  ...
  assert outcome=200, 'Wrong total speed award';
  assert (select score=100 from public.teams where id=team_a), 'Fast team score wrong';
  assert (select score=75 from public.teams where id=team_b), 'Edited team score wrong';
  assert (select score=25 from public.teams where id=team_c), 'Partial team score wrong';
  ...
  perform public.rescore_submission(s3,'{"items":[{"status":"correct"},{"status":"correct"}]}',2);
  assert (select score=50 from public.teams where id=team_c), 'Rescore lost timing or duplicated points';
  ...
  assert (select score=200 from public.teams where id=team_a), 'Bonus score conversion failed';
  perform public.rescore_bonus_submission(bonus_id,'{"items":[{"status":"incorrect"}]}',0);
  assert (select score=100 from public.teams where id=team_a), 'Bonus correction failed';
  ...
  assert (select score=100 from public.teams where id=team_a), 'Duplicate reveal inflated points';
  ...
  perform public.resolve_spin_the_wheel(show_id);
  assert (select score=200 from public.teams where id=team_a), 'Game winner did not receive exactly 100 points';
  insert into public.game_show_games(game_id,show_game_key,item_position,round_number,round_title,game_type,title,settings)
  ...
  assert (select score=200 from public.teams where id=team_b), 'Composite scorer did not convert both stages';
  select game_id into game from public.create_game_from_quiz(quiz,'{"scoring_mode":"speed"}');
  update public.games set status='live',current_screen='multi-answer',answer_phase='open' where id=game;
  ...
  assert (select score=100 from public.teams where id=team_b), 'Auto-Run did not convert points';
