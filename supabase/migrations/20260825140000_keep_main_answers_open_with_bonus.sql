-- Revealing a bonus adds a second response field without locking the ordinary
-- response. Both remain editable until the host closes the question.
...
begin
  ...
  where games.id = p_game_id
    ...
    and games.question_stage in ('core', 'bonus')
...
comment on function public.submit_player_answer(uuid, uuid, text) is
  'Creates or replaces the active ordinary response while the shared main-and-bonus answer window remains open.';
