...
-- Game-only shows have no question key to use while their first show game is
-- active. The application and generated database types already model this
-- field as nullable, so align the deployed legacy column with that contract.
alter table public.games
  alter column current_question_key drop not null;

-- A deliberate half-second hold is required before treasure starts accruing.
-- Keeping the warm-up in this server-authoritative function means quick taps
-- cannot score even when the client is slow or modified.
...
begin
  ...
  if p_holding then
    ...
    if result.is_stealing and not guard_awake then
      earned := greatest(
        0,
        floor((extract(epoch from (clock_timestamp() - result.stealing_started_at)) - 0.5) * 1000)::bigint
      );
      update public.game_show_game_treasure
...
-- If time expires while a team is holding, apply the same warm-up before
-- banking their final safe haul.
...
begin
  ...
  if not coalesce((result.settings->>'guard_awake')::boolean, false) then
    ...
    set banked_units = banked_units + greatest(
          0,
          floor((extract(epoch from (clock_timestamp() - stealing_started_at)) - 0.5) * 1000)::bigint
        ),
        is_stealing = false,
        stealing_started_at = null,
        current_units = 0,
...
comment on function public.set_steal_the_treasure_holding(uuid, uuid, uuid, boolean) is
  'Starts or banks a player treasure attempt, with a server-enforced 500ms warm-up before treasure accrues.';
