-- games.current_question_key is not nullable for legacy player state.  Keep
-- the first frozen question as a harmless fallback while a content screen or
-- show game is the active first item.
...
begin
  ...
  select question_key into fallback_question_key
  from public.game_questions
  where game_id = live_game.id
  order by item_position
  limit 1;

  -- Game-only shows are allowed when their games can award points. Use an
  -- internal placeholder only where the legacy column still requires one.
  if fallback_question_key is null then
    fallback_question_key := live_game.current_question_key;
  end if;
  ...
  set status = 'live',
      ...
      question_stage = 'core',
      current_question_key = case when first_item.kind = 'question' then first_item.item_key else fallback_question_key end,
...
comment on function public.start_live_game(uuid, boolean) is
  'Atomically starts a host-owned lobby from its first item while retaining a legacy question-key fallback for game-first shows.';
